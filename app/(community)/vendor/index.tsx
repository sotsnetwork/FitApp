...
export default function VendorCommunity() {
  ...
  const pickImage = async () => {
    ...
    router.push({ pathname: '/(vendor)/create-post', params: { type: 'picture' } });
  };

  const pickVideo = async () => {
    setCreatePostModalVisible(false);
    router.push({ pathname: '/(vendor)/create-post', params: { type: 'video' } });
  };
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: 'white' }}>
      ...
      <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingTop: 0 }}>
        <View style={{ paddingHorizontal: spacing.lg, paddingTop: 0, paddingBottom: spacing.lg, marginTop: -4 }}>
          {selectedTab === 'Challenges' ? (
            <>
              ...
              {[1,2,3].map((i) => (
                <TouchableOpacity key={i} onPress={() => router.push('/(community)/vendor/challenge-details')} style={{ flexDirection: 'row', alignItems: 'center', paddingVertical: spacing.md, borderBottomWidth: 1, borderBottomColor: colors.border }}>
        ...
        {/* Feed Posts */}
          ...
          <View style={{ marginBottom: spacing.md, marginTop: 0 }}>
            ...
            <View style={{ flexDirection: 'row', gap: spacing.md }}>
              ...
              <TouchableOpacity
                onPress={() => router.push('/(vendor)/post-detail')}
                style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.xs }}
              >
        ...
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.4)', justifyContent: 'flex-end' }}>
          ...
          <View style={{ backgroundColor: 'white', borderTopLeftRadius: 20, borderTopRightRadius: 20, padding: spacing.xl }}>
            ...
            <TouchableOpacity
              onPress={() => {
                setCreatePostModalVisible(false);
                router.push({ pathname: '/(vendor)/create-post', params: { type: 'text' } });
              }}
              style={{
                flexDirection: 'row',
                alignItems: 'center',
